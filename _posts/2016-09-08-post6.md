...
<h2 id="media">2、reduce操作的时候，进行结果集迭代注意事项，Iterable<T> values迭代产生的元素为单例模式，因此我们需要进行实例的拷贝</h2>
<p>
    程序中Iterable<T>接口的实现为org.apache.hadoop.mapreduce.task.ReduceContextImpl$ValueIterable，其特殊性参见链接：
    http://my.oschina.net/shipley/blog/603386</br>
    另外，Reducer接口API中也对reduce方法进行了相应阐述。</br>
    http://hadoop.apache.org/docs/r2.6.0/api/index.html?org/apache/hadoop/mapred/Reducer.html
